...
for file in listFiles:
    print(file)
    dfBuffer = pd.read_excel("%s"%file, sheet_name="Other campaigns")
    dfBuffer["File"] = file
...
for file in listFiles:
    print(file)
    try:
        dfBuffer2 = pd.read_excel("%s"%file, sheet_name="Twitter campaigns")
        dfBuffer2["File"] = file
...
mediacomData = inData[~inData["Campaign"].str.contains("_BRA_")]
brasilData = inData[inData["Campaign"].str.contains("_BRA_")]
